...
const worldBackground = document.getElementById('world-background') as HTMLElement;
const socket = io();
...
const users: { [id: string]: { star: HTMLElement, username: string, speechBubble: HTMLElement, x: number, y: number, connectedTime: number } } = {};
const userListElement = document.getElementById('userList') as HTMLElement;
const userListBody = document.getElementById('userListBody') as HTMLElement;
...
function createStar(id: string, username: string, x: number, y: number): HTMLElement {
...
function updateStarPosition(starElement: HTMLElement, worldX: number, worldY: number): void {
...
socket.on('connect', () => {
  ...
  socket.emit('userJoined', currentUsername);
    ...
    socket.on('currentUsers', (serverUsers: { [id: string]: { x: number, y: number, username: string } }) => {
  for (const id in serverUsers) {
    if (!users[id]) {
      const user = serverUsers[id];
      const star = createStar(id, user.username, user.x, user.y);
      const speechBubble = star.querySelector('.speech-bubble') as HTMLElement;
...
socket.on('userConnected', (data: { id: string, data: { x: number, y: number, username: string } }) => {
  if (!users[data.id]) {
...
socket.on('starMoved', (data: { id: string, position: { x: number, y: number } }) => {
  if (users[data.id]) {
...
function initializeLocalStarMovement() {
  ...
  window.addEventListener('keydown', (e: KeyboardEvent) => { pressedKeys.add(e.key); });
  window.addEventListener('keyup', (e: KeyboardEvent) => { pressedKeys.delete(e.key); });

  function checkCollision(star1: HTMLElement, star2: HTMLElement): boolean {
  ...
  function gameLoop() {
    if (users[socket.id]) {
        ...
        if (pressedKeys.has('ArrowUp')) dy -= speed;
        if (pressedKeys.has('ArrowDown')) dy += speed;
        if (pressedKeys.has('ArrowLeft')) dx -= speed;
        if (pressedKeys.has('ArrowRight')) dx += speed;
        ...
        if (dx !== 0 || dy !== 0) {
            ...
            for (const id in users) {
                if (id === socket.id) continue; // Don't check collision with self
...
socket.on('chatMessage', (data: { id: string, username: string, message: string }) => {
